...
const StaffLogin = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('https://green-kerala-api.onrender.com/api/staffapp/staff-login/', {
                staff_id: parseInt(staffId),
                password: password // Send password instead of name
            });
            ...
            if (response.data.success) {
                ...
                setError(response.data.message || 'Invalid Staff ID or Password');
            }
        } catch (error) {
            console.error('Login error:', error);
            setError(error.response?.data?.error || 'Login failed. Please try again.');
                    ...
                    <div className="card shadow-sm border-0 rounded-4">
                        <div className="card-body p-5">
                            <div className="text-center mb-4">
                                <div className="display-1">🔒</div>
                                <h2 className="fw-bold">Staff Login</h2>
                                <p className="text-muted">Secure Staff Portal</p>
                            </div>
                            {error && <div className="alert alert-danger">{error}</div>}
                            <form onSubmit={handleSubmit}>
                                <div className="mb-3">
