...
const About = () => {
    ...
    const fetchRealData = async () => {
        try {
            ...
            // Fetch pickups data for waste collected
            const pickupsRes = await axios.get('https://green-kerala-api.onrender.com/api/pickups/');
            let pickupsList = [];
            if (pickupsRes.data.results) {
                pickupsList = pickupsRes.data.results;
            } else if (Array.isArray(pickupsRes.data)) {
                pickupsList = pickupsRes.data;
            }

            // Calculate waste collected (estimated)
            const wasteWeightMap = {
                '1-5': 3,
                '5-10': 7.5,
                '10-20': 15,
                '20+': 25
            };
            const totalWaste = pickupsList.reduce((sum, pickup) => {
                return sum + (wasteWeightMap[pickup.estimated_weight] || 0);
            }, 0);
            ...
            const completedMissions = missionsList.filter(m => m.status === 'completed').length;

            setStats({
                activeVolunteers: volunteers.length,
                missionsCompleted: completedMissions || missionsList.length,
                treesPlanted: treesPlantedApprox,
                partnerNGOs: 12,
                wasteCollected: totalWaste,
                happyVolunteers: volunteers.filter(v => (v.total_points || 0) > 0).length
            });
            ...
            // Fallback values
            setStats({
                activeVolunteers: 1250,
                missionsCompleted: 85,
                treesPlanted: 5000,
                partnerNGOs: 12,
                wasteCollected: 10000,
                happyVolunteers: 2500
            });
        } finally {
            setLoading(false);
        }
    ...
    const statsDisplay = [
        { number: loading ? "..." : `${stats.activeVolunteers}+`, label: "Active Volunteers", icon: "👥", color: "#4CAF50" },
        { number: loading ? "..." : `${stats.missionsCompleted}+`, label: "Missions Completed", icon: "🎯", color: "#2196F3" },
        { number: loading ? "..." : `${stats.treesPlanted.toLocaleString()}+`, label: "Trees Planted", icon: "🌳", color: "#8BC34A" },
        { number: `${stats.partnerNGOs}+`, label: "Partner NGOs", icon: "🤝", color: "#FF9800" },
        { number: loading ? "..." : `${stats.wasteCollected.toLocaleString()}+`, label: "KG Waste Collected", icon: "♻️", color: "#9C27B0" },
        { number: loading ? "..." : `${stats.happyVolunteers}+`, label: "Happy Volunteers", icon: "😊", color: "#E91E63" }
    ];
                                ...
                                <div className="card-body p-5 text-center">
                                    ...
                                    <div className="row mt-4">
                                        <div className="col-md-3 col-6 mb-3">
                                            <h2 className="fw-bold text-success">{loading ? '...' : `${stats.activeVolunteers}+`}</h2>
                                            <p>Active Volunteers</p>
                                        </div>
                                        <div className="col-md-3 col-6 mb-3">
                                            <h2 className="fw-bold text-success">{loading ? '...' : `${stats.missionsCompleted}+`}</h2>
                                            <p>Missions</p>
                                        </div>
                                        <div className="col-md-3 col-6 mb-3">
                                            <h2 className="fw-bold text-success">{loading ? '...' : `${stats.treesPlanted.toLocaleString()}+`}</h2>
                                            <p>Trees Planted</p>
                                        </div>
                                        <div className="col-md-3 col-6 mb-3">
                                            <h2 className="fw-bold text-success">{loading ? '...' : `${stats.wasteCollected.toLocaleString()}+`}</h2>
